...
func HueOf(linrgb []float64) float64 {
	scaledDiscount := MatrixMultiply(linrgb, SCALED_DISCOUNT_FROM_LINRGB)
		...
		if left[axis] != right[axis] {
			...
			for i := 0; i < 8; i++ {
				if math.Abs(float64(rPlane-lPlane)) <= 1 {
					...
					midPlaneCoordinate := CRITICAL_PLANES[mPlane]
...
func FindResultByJ(hueRadians float64, chroma float64, y float64) int {
	...
	tInnerCoeff := 1.0 / math.Pow(1.64-math.Pow(0.29, viewingConditions.N()), 0.73)
	...
	for iterationRound := 0; iterationRound < 5; iterationRound++ {
		...
		linrgb := MatrixMultiply(
			[]float64{rCScaled, gCScaled, bCScaled}, LINRGB_FROM_SCALED_DISCOUNT)
		// ===========================================================
		// Operations inlined from Cam16 to avoid repeated calculation
		// ===========================================================
